...
divBlock.innerText = 'My test DIV'
...
divBlock.style.backgroundColor = 'silver'
divBlock.style.color = 'red'
divBlock.style.fontSize = '40px'
divBlock.style.marginBottom = '7px'
// - додати цей блок в body.
document.body.appendChild(divBlock)
// - клонувати його повністю, та додати клон в body.
let clone = divBlock.cloneNode(true)
...
for (const cadaElement of coursesAndDurationArray) {
    const value = document.createElement('div');
    value.innerText = `${cadaElement.title} ${cadaElement.monthDuration}`
...
for (const cadaElement of coursesAndDurationArray) {
    ...
    h1.innerText = `${cadaElement.title}`
    let p = document.createElement('p')
    p.className = 'description'
    p.innerText = `${cadaElement.monthDuration}`
...
for (const simpson of simpsons) {
    ...
    const simAge = document.createElement('p');
    simAge.innerText = `Age:${simpson.age}`
    const simInfo = document.createElement('p');
    simInfo.innerText = `Info:${simpson.info}`

    const img = document.createElement('img');
    img.src = `${simpson.photo}`

    DivElement.append(h3,simAge,simInfo,img)
}
...
for (const Element of coursesArray) {
    ...
    const modulesList = document.createElement('ol');
    for (const module of Element.modules) {
        const valueList = document.createElement('li');
        valueList.innerText = `${module}`
        modulesList.appendChild(valueList)
    }
    modulesBlock.appendChild(modulesList)
    titlesBlock.append(monthBlock,hourBlock)
    arrBlock.append(titleBlock,titlesBlock,modulesBlock)
